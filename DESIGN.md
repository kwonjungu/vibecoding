...
- **가운데 두는 것** — `.lecture-detail`, `.ladder-note`, 그리고 `.guide-key` 의 머리(`h3`·`.key-lead`)와 맺음말(`.key-foot`). `.guide-key` 는 그 자체로 한 덩어리라 머리를 가운데 두면 구간이 나뉘는 것이 보인다. 본문 안의 `.lecture-actions` 버튼도 가운데로 모은다. 왼쪽에 홀로 놓인 버튼은 길 잃은 것처럼 보인다.
- **왼쪽에 맞추는 것** — `.hero .lead`, `.pane .lead`. 바로 위 제목과 왼쪽 모서리를 맞춘다.
- **언제나 왼쪽 정렬** — 문단으로 읽는 글. `.key-text`, `.detail-block` 의 본문, `<li>`. 여러 줄이 되는 글을 가운데 정렬하면 줄마다 시작점이 달라져 읽기 어렵다.

예외는 `.trait-call` 하나다. §5를 보라.
