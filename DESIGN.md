...
- 사다리 지도 아래에 "모두 펼치기" 버튼 하나를 둔다. 인쇄하거나 전체를 훑을 때 쓴다. 라벨은 버튼이 기억하지 않고 **그때그때 칸들의 실제 상태에서 끌어낸다** — 하나라도 접혀 있으면 "모두 펼치기", 전부 펼쳐져 있으면 "모두 접기". 개별 토글로 몇 개만 펼친 뒤 눌러도 한 번에 반응한다.
